...
module.exports = function(wave, note, options) {
  ...
  if (options.linear || options.exp) {
    ...
    if (options.linear || cycles < LSDJ.constants.SAMPLES_PER_FRAMES) {
      if (options.exp) {
      ...
      sample = 0
      for (cycle = 0; cycle < LSDJ.constants.FRAMES_PER_TABLE; cycle++) {
        sample = ~~(cycleInterval) * cycle * LSDJ.constants.SAMPLES_PER_FRAMES
        for (nextSample=0; nextSample < LSDJ.constants.SAMPLES_PER_FRAMES; nextSample++) {
          // console.log(sample)
